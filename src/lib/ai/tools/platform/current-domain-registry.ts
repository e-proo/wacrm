...
const SPECS: readonly CurrentToolSpec[] = [
  ...
  },
  {
    ...
    planes: ['customer', 'admin'], capability: 'coverage.read', purpose: 'Search customer-safe active coverage availability.',
    use: ['A requester needs matching coverage options for a known service/currency/region.'],
    avoid: ['Do not expose raw provider identity/cost on customer plane.'], constraints: ['service_ids', 'currencies'],
  },
  {
    key: 'coverage.get_rates', version: 1, domain: 'coverage', title: 'Read coverage rate board', permission: 'read', risk: 'read',
    planes: ['customer', 'admin'], capability: 'coverage.read', purpose: 'Read the current published coverage commission board.',
    use: ['The user asks for published coverage/remittance/cash rates.'],
    avoid: ['Do not invent a missing unpublished rate.'],
  },
  {
    key: 'coverage.propose_offer', version: 2, domain: 'coverage', title: 'Forward customer coverage offer', permission: 'propose', risk: 'medium',
    planes: ['customer'], capability: 'coverage.propose', purpose: 'Create a typed approval proposal from the current customer’s coverage offer.',
    use: ['The customer is offering liquidity and required offer fields are complete.'],
    avoid: ['Never let the model choose customer identity and never activate an offer directly.'], constraints: ['service_ids', 'currencies'],
  },
  {
    key: 'coverage.propose_request', version: 1, domain: 'coverage', title: 'Forward customer coverage request', permission: 'propose', risk: 'medium',
    planes: ['customer'], capability: 'coverage.propose', purpose: 'Create a typed approval proposal from the current customer’s coverage request.',
    use: ['The customer needs coverage and required request fields are complete.'],
    avoid: ['Never let the model choose customer identity and never create the authoritative request directly.'], constraints: ['service_ids', 'currencies'],
  },
  {
