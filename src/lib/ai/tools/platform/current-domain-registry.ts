...
const SPECS: readonly CurrentToolSpec[] = [
  ...
  },
  {
    ...
    purpose: 'Search canonical coverage supply using both customer legs; a north-pay to south-receive request matches anti-parallel south-pay to north-receive offers.',
    use: ['The customer pays in the north, receives in the south, and the service/currency/amount plus both leg regions are known.'],
    avoid: ['Do not use a south-pay to north-receive customer case as a request; that direction is an offer. Never expose provider identity/internal cost on customer plane.'], constraints: ['service_ids', 'currencies'],
  },
  {
    ...
    planes: ['customer', 'admin'], capability: 'coverage.read',
    purpose: 'Resolve pay/receive regions, classify the canonical north/south coverage direction, read the published rate, and calculate the commission amount.',
    use: ['Use first for a concrete coverage question: south-pay to north-receive is an offer with commission returned to customer; north-pay to south-receive is a request with commission paid by customer.'],
    avoid: ['Do not infer offer/request from the words راجع or عمولة. Do not invent a missing unpublished rate.'],
  },
  {
    ...
    avoid: ['Never classify from wording alone, never let the model choose customer identity, and never trust a model-supplied commission over the published board.'], constraints: ['service_ids', 'currencies'],
  },
  {
