...
describe('tool grant plane inheritance policy', () => {
  ...
  it('drops admin-only tools from customer drafts', () => {
    for (const tool of [
      ...
      grant('exchange_rates.admin_list_pairs', 1, 'read'),
      grant('exchange_rates.admin_list_trade_requests', 1, 'read'),
      grant('exchange_rates.propose_pair_change', 2, 'propose'),
      grant('exchange_rates.propose_trade_decision', 1, 'propose'),
