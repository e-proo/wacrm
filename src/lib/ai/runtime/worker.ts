...
/**
 * One bounded worker tick. It is safe to run concurrently: every candidate is
 * finally protected by claim_agent_run's lease/CAS. The scheduler calls this
 * repeatedly; no in-process infinite loop is required in serverless runtimes.
 */
export async function processCustomerIntentNotifications(input: {
  ...
  const { data: candidates, error } = await db
    ...
    .select('id, account_id, contact_id, conversation_id, message_text, attempts')
    .eq('status', 'pending')
  ...
  for (const raw of candidates ?? []) {
    ...
    try {
      ...
      const outbound = await engineSendText({
        ...
        engineIdempotencyKey: `intent-notification:${row.id}`,
      })
      ...
      // a timeout can happen after Meta accepted the message. Human/receipt
      // reconciliation decides whether it is safe to resend.
