...
import {
  ...
  setFxBusinessEventDeliveryMode,
} from './cutover'

const accountId = process.env.WACRM_FX_CUTOVER_LIVE_ACCOUNT_ID

const activateEnabled =
  process.env.WACRM_FX_CUTOVER_ACTIVATE_LIVE === '1'
const rollbackEnabled =
  process.env.WACRM_FX_CUTOVER_ROLLBACK_LIVE === '1'
...
activateDescribe('FX controlled cutover activation on TEST', () => {
  it('activates only after a successful readiness preflight', async () => {
    if (process.env.WACRM_FX_CUTOVER_CONFIRM !== 'ACTIVATE_TEST') {
      throw new Error('WACRM_FX_CUTOVER_ACTIVATION_CONFIRMATION_REQUIRED')
    }

    const resolvedAccountId = requireLiveAccount()
    const before = await inspectFxBusinessEventCutoverReadiness({
    ...
    const activated = await setFxBusinessEventDeliveryMode({
      accountId: resolvedAccountId,
      mode: 'active',
    })
    ...
    try {
      const after = await inspectFxBusinessEventCutoverReadiness({
        accountId: resolvedAccountId,
      })
      ...
      await setFxBusinessEventDeliveryMode({
        accountId: resolvedAccountId,
        mode: 'legacy',
      }).catch(() => undefined)
      throw error
...
rollbackDescribe('FX controlled cutover rollback on TEST', () => {
  it('returns the route to legacy through the guarded rollback RPC', async () => {
    if (process.env.WACRM_FX_CUTOVER_CONFIRM !== 'ROLLBACK_TEST') {
      throw new Error('WACRM_FX_CUTOVER_ROLLBACK_CONFIRMATION_REQUIRED')
    }

    const resolvedAccountId = requireLiveAccount()
    const rolledBack = await setFxBusinessEventDeliveryMode({
      accountId: resolvedAccountId,
      mode: 'legacy',
    })
