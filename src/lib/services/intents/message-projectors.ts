...
import type { IntentBusinessEventType } from './business-events'
...
type ServiceRequestOutcome =
  (typeof OUTCOME_BY_EVENT)[IntentBusinessEventType]
...
export function buildIntentBusinessEventProjection(
  ...
  const eventType = input.eventType as IntentBusinessEventType
...
const projector = async (
  ...
  buildIntentBusinessEventProjection(input)

export const INTENT_EVENT_PROJECTORS: readonly EventProjectorRegistration[] = (
  Object.keys(OUTCOME_BY_EVENT) as IntentBusinessEventType[]
).map((eventType) => ({
  eventType,
  eventVersion: 1,
  projector,
